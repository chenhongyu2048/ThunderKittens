...
struct globals {
    ...
    using b_tile = st_int8<C::Nb/2, C::Kb>;
    using d_tile = st_int<C::Mb/2, C::Nb/C::EPI_PIPE_DEPTH>;
...
__global__ void kernel(const __grid_constant__ globals<C> g) {
    ...
    using d_tt_t = tt<int, C::Mb/2, C::Nb>;
    ...
    if (warpgroup::groupid() == C::NUM_CONSUMERS) {
        ...
        for(int task_iter = 0; true; task_iter++) {
            ...
            if constexpr (C::OVERLAP_MMA_EPI) {
                rt_int<C::Mb/8, C::Nb/C::EPI_PIPE_DEPTH> d_reg;
                #pragma unroll
                for(int i = 0; i < C::EPI_PIPE_DEPTH; i++) {
                    warpgroup::load_async(d_reg, d_tt[task_iter%C::MMA_PIPE_DEPTH].template subtile<tt<int, C::Mb/2, C::Nb/C::EPI_PIPE_DEPTH>>(0, C::Nb/C::EPI_PIPE_DEPTH*i));
                ...
                rt_int<C::Mb/8, C::Nb/C::EPI_PIPE_DEPTH> d_reg[C::EPI_PIPE_DEPTH];
                #pragma unroll
                for(int i = 0; i < C::EPI_PIPE_DEPTH; i++)
                    warpgroup::load_async(d_reg[i], d_tt[task_iter%C::MMA_PIPE_DEPTH].template subtile<tt<int, C::Mb/2, C::Nb/C::EPI_PIPE_DEPTH>>(0, C::Nb/C::EPI_PIPE_DEPTH*i));
                tensor_load_wait();
                warpgroup::sync(warpgroup::groupid()+1);
                if (!schedule.success) warpgroup::pdl::arrive();
                warpgroup::tma::cluster::arrive(outputs_finished[task_iter%C::MMA_PIPE_DEPTH], 0);
...
__host__ double run_benchmark(size_t M, size_t N, size_t K, bool ncu = false) {
    ...
    std::vector<int*> d_C(arg_group_count);
    int* d_C_ref;
    for (int i = 0; i < arg_group_count; i++) {
        CUDACHECK(cudaMalloc(&d_A[i], M*K*sizeof(int8)));
        CUDACHECK(cudaMalloc(&d_B[i], K*N*sizeof(int8)));
        CUDACHECK(cudaMalloc(&d_C[i], M*N*sizeof(int)));
    }
    CUDACHECK(cudaMalloc(&d_C_ref, M*N*sizeof(int)));
    std::cout << "Allocated device memory" << std::endl;

    // Initialize matrices with small integers so the float reference accumulates exactly.
    uint64_t seed = 2024;
    for (int i = 0; i < arg_group_count; i++) {
        ...
        fill<int, FillMode::CONSTANT>(d_C[i], M*N, 0.0f);
    }
    fill<int, FillMode::CONSTANT>(d_C_ref, M*N, 0.0f);
    CUDACHECK(cudaDeviceSynchronize());
    std::cout << "Initialized matrices on device" << std::endl;

    // Compute reference GEMM on device
    reference_gemm<int8, int>(d_C_ref, d_A[0], d_B[0], M, N, K);
    CUDACHECK(cudaDeviceSynchronize());
...
__host__ int main() {
    ...
    N = 1024;
    run_benchmark<config<256, 64, 256, 4, true, 4, 2>>(N, N, N, ncu);
    N = 2048;
    run_benchmark<config<256, 256, 128, 8, true, 4, 4>>(N, N, N, ncu);
    N = 4096;
    run_benchmark<config<256, 256, 128, 4, true, 4, 4>>(N, N, N, ncu);
    N = 8192;
    run_benchmark<config<256, 256, 128, 8, false, 3, 8>>(N, N, N, ncu);
    N = 16384;
    run_benchmark<config<256, 256, 128, 8, false, 3, 8>>(N, N, N, ncu);
